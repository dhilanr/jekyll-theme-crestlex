...
require "jekyll-theme-crestlex/constants"
...
module CRESTLEX
  ...
  def self.convert(s)
    markdown, indentation = CRESTLEX::unindent(s)
    html = $site.find_converter_instance(::Jekyll::Converters::Markdown).convert(markdown).strip
    CRESTLEX::indent(html, indentation)
  end

  # Format Time for time.CRESTLEX.io
  ...
  class AfterBeforeBlock < Block
    ...
    def render(context)
      html = CRESTLEX::convert(super)

      # Parse timestamp
      iso8601 = CRESTLEX::strptime(@args[0]).iso8601
  ...
  class CalendarTag < Tag
    ...
    def render(context)
      ...
      # Deprecated @ctz
      if @kwargs.key?("ctz")
        Jekyll.logger.warn "CRESTLEX warning: no need for @ctz anymore"
      end
  ...
  class LocalTag < Tag
    ...
    def render(context)
      ...
      t1 = CRESTLEX::strptime(@args[0])
      local = t1.iso8601
      href = "https://time.CRESTLEX.io/" + CRESTLEX::format(t1)

      # Parse optional argument
      if @args.length == 2
        t2 = CRESTLEX::strptime(@args[1])
        if t2 < t1
          raise "Invalid interval: #{@markup}"
        end
        local += "/" + t2.iso8601
        href += "/" + CRESTLEX::format(t2)
      end
  ...
  class NextTag < Tag
    ...
    def render(context)
      super
      button = CRESTLEX::sanitize(CRESTLEX::convert((@args[0]) ? CGI.escapeHTML(@args[0]) : "Next"))
  ...
  class SpoilerBlock < Block
    ...
    def render(context)
      html = CRESTLEX::convert(super)
      summary = CRESTLEX::sanitize(CRESTLEX::convert((@args[0]) ? CGI.escapeHTML(@args[0]) : "Spoiler"))
      "<details>" \
  ...
  class VideoTag < Tag
    ...
    def render(context)
      ...
      if @args[0] 
        ...
        # If CRESTLEX Video Player
        elsif @args[0] =~ /^https?:\/\/video\.CRESTLEX\.io\/([^?]+)/
  ...
  module Filters
    def relative_url(input)
      Jekyll.logger.warn "CRESTLEX warning: no need to use relative_url with this theme"
      input
    end
  end
  Liquid::Template.register_filter(CRESTLEX::Filters)
...
Jekyll::Hooks.register :site, :after_reset do |site|
  ...
  if site.config.key?("plugins") and site.config["plugins"].kind_of?(Array) and site.config["plugins"].include? "jekyll-relative-links"
    ...
    Jekyll.logger.warn "CRESTLEX warning: no need to use jekyll-relative-links with this theme"
  end

  # Merge in theme's configuration
  site.config = CRESTLEX::DEFAULTS.dup.deep_merge!(site.config).deep_merge!(CRESTLEX::OVERRIDES)
end
...
Jekyll::Hooks.register :site, :pre_render do |site, payload|
  ...
  ENV["TZ"] = site.config["CRESTLEX"]["tz"]

  # Promote site.CRESTLEX.assign.* to global variables
  begin
    site.config["CRESTLEX"]["assign"].each do |key, value|
      payload[key] = value
    end
  rescue
  end
